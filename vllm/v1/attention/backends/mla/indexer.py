...
def _prepare_uniform_decode_kernel(
    ...
    for i in tl.range(0, expanded_bt_stride, BLOCK_SIZE):
        ...
        dst_mask = off < expanded_bt_stride
        src_mask = off < block_table_stride
        src_block = tl.load(src + off, mask=src_mask, other=0)
        tl.store(dst + off, src_block, mask=dst_mask)
...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Allocate at the final expected row width before CUDA graph capture.
        # Replacing this workspace later can leave captured graphs or registered
        # kernel addresses pointing at the previous tensor.
        max_num_blocks_per_req = self._max_expanded_block_table_width(
            self.vllm_config,
            self.kv_cache_spec,
            self.num_speculative_tokens,
        )
    ...
    @staticmethod
    def _max_expanded_block_table_width(
        vllm_config: VllmConfig,
        kv_cache_spec: AttentionSpec,
        ...
        max_block_table_len = max(
            ...
            vllm_config.scheduler_config.max_num_encoder_input_tokens or 0,
        )
        max_num_blocks_per_req = kv_cache_spec.max_num_blocks_per_req(
            vllm_config, max_block_table_len
        )
        ...
        if kv_cache_spec.block_size <= 128:
            block_table_alignment = 128 // kv_cache_spec.block_size
            max_num_blocks_per_req = (
                cdiv(max_num_blocks_per_req, block_table_alignment)
                * block_table_alignment
            )
        return max_num_blocks_per_req

    def _expanded_block_table(self, block_table_width: int) -> torch.Tensor:
        """Return the graph-stable expanded-block-table workspace.

        The workspace is allocated in ``__init__`` at a conservative aligned
        width. Decode paths may receive a narrower runner block table; they copy
        that prefix and zero-fill the remaining columns. A wider runner table
        means the up-front sizing formula is wrong and must fail before CUDA
        kernels can read/write out of bounds.
        """
        if self.expanded_block_table_buffer.shape[1] < block_table_width:
            raise RuntimeError(
                "Sparse MLA indexer expanded block table is too narrow: "
                f"allocated width {self.expanded_block_table_buffer.shape[1]}, "
                f"runner width {block_table_width}."
            )
    ...
    def _prepare_decode_tensors(
        ...
        if not use_native and max_decode_len > 1:
            ...
            if min_decode_len == max_decode_len:
                ...
                expanded_block_table = self._expanded_block_table(block_table.shape[1])
                ...
                # original request.
                expanded_block_table = self._expanded_block_table(block_table.shape[1])
                expanded_block_table[:actual_expanded, : block_table.shape[1]] = (
                ...
                if block_table.shape[1] < expanded_block_table.shape[1]:
                    expanded_block_table[:actual_expanded, block_table.shape[1] :] = 0
                if actual_expanded < num_decode_tokens:
                    expanded_block_table[actual_expanded:num_decode_tokens] = 0
                block_table = expanded_block_table[:num_decode_tokens]
